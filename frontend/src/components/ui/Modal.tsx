...
export function Modal({ title, open, onClose, children }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 grid items-end bg-black/25 p-0 sm:place-items-center sm:p-4">
      <motion.div initial={{ opacity: 0, scale: 0.98, y: 28 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="glass max-h-[88svh] w-full overflow-y-auto rounded-t-[2rem] p-5 sm:max-w-2xl sm:rounded-3xl">
        <div className="mb-5 flex items-center justify-between">
          ...
          <button onClick={onClose} className="rounded-2xl border border-gray-200 p-2 text-gray-600 hover:bg-[#F9FAFB]" aria-label="Close modal">
            <X size={18} />
          </button>
        </div>
        {children}
