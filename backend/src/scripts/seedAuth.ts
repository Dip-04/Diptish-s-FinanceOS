...
async function seedFinanceData(userId: string) {
  ...
  if (incomeError) console.warn(`Income seed skipped: ${incomeError.message}`)
  ...
  if (expenseError) console.warn(`Expense seed skipped: ${expenseError.message}`)
  ...
  if (loanError) console.warn(`Loan seed skipped: ${loanError.message}`)
  ...
  if (goalError) console.warn(`Goal seed skipped: ${goalError.message}`)

  await seedMonthlyPlans(userId)
}

async function resetDemoFinanceData(userId: string) {
  const tables = ['savings_goals', 'loans', 'expenses', 'income_sources'] as const
  for (const table of tables) {
    const { error } = await supabase!.from(table).delete().eq('user_id', userId)
    if (error) console.warn(`${table} reset skipped: ${error.message}`)
  }
...
async function seedMonthlyPlans(userId: string) {
  const plans = [
  ...
  for (const plan of plans) {
    ...
    if (itemError) console.warn(`Monthly plan items seed skipped: ${itemError.message}`)
  }
}
