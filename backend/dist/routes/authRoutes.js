...
authRoutes.get('/profile', asyncHandler(async (req, res) => {
    ...
    if (!supabase || !userId)
        return res.json({ id: 'local-user', name: 'Diptish Gohane', email: 'diptish@example.com' });
...
authRoutes.put('/profile', asyncHandler(async (req, res) => {
    ...
    if (!supabase || !payload.id)
        return res.json({ id: payload.id ?? 'local-user', ...req.body, updated_at: new Date().toISOString() });
